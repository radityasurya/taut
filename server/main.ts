...
const hostname = process.env.TAUTAN_BIND ?? '127.0.0.1';
...
console.log(`tautan: http://${hostname}:${port}  muxes: ${[...muxes.map(mux => mux.id), ...tmuxes.map(t => `tmux-${t.id}`)].join(', ') || 'none'}`);
for (const item of discovered) console.log(`tautan: ${item.id} ${item.socketPath}`);
if (!muxes.length && !tmuxes.length) console.warn('tautan: warning: no Muxes found');
void syncHosts(hub).catch(error => console.warn('tautan: remote Host discovery failed', error));
